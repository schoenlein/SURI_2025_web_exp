...
jsPsych.plugins['survey-text'] = (function() {
  ...
  plugin.info = {
    ...
    parameters: {
      ...
      passwordType: {
        type: jsPsych.plugins.parameterType.INT,
        pretty_name: 'whether a password or not',
        default:  0,
        description: '.'
      }
    }
  ...
  plugin.trial = function(display_element, trial) {
    ...
    for (var i = 0; i < trial.questions.length; i++) {
      ...
      if(question.rows == 1){
       ...
       }else if (trial.passwordType == 1){
        html += '<input type="password" id="input-'+question_index+'"  name="#jspsych-survey-text-response-' + question_index + '" data-name="'+question.name+'" size="'+question.columns+'" '+autofocus+' '+req+' placeholder="'+question.placeholder+'"></input>';
       }
      } else {

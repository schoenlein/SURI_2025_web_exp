...
jsPsych.plugins["image-keyboard-response-instrMAS-randomColorGrid"] = (function() {

  var plugin = {};

  jsPsych.pluginAPI.registerPreload('image-keyboard-response-instrMAS-randomColorGrid', 'stimulus', 'image');

  plugin.info = {
    name: 'image-keyboard-response-instrMAS-randomColorGrid',
    description: '',
    parameters: {
      stimulus: {
  ...
  plugin.trial = function(display_element, trial) {
    ...
    var html = "";
         ...
         if (trial.prompt1 !== null) {
          // html += '<span style="font-size: 30px; display:inline-block; width: 800px;">'+trial.prompt+'</span>'
           html += trial.prompt1;
          }
 ...
 /*   // display stimulus
  ...
  //For j rows
   for (j = 0; j < 3; j++){
       ...
       //IMAGES (for i examples per rows)
       if (j == 1){

        var shift = -120; 
        for (i = 0; i < 10; i++){
          shift = shift +20; 
          html += '<img src="'+trial.stimulus[i+set]+'";  id="jspsych-image-keyboard-response-stimulus"; style ="width: 40px; position: relative; left:'+shift+'px;" '; 
          html += '"></img>'; 
          }   
          set = set + 10  
      }else{
    ...
    if (trial.prompt2 !== null) {
      html += trial.prompt2
      //html += '<span style="font-size: 120%;">'+trial.prompt2+'</span>'
    }
